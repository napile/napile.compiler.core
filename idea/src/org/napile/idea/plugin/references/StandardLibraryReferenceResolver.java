...
public class StandardLibraryReferenceResolver extends AbstractProjectComponent
{
	...
	private void ensureInitialized()
	{
		synchronized(lock)
		{
			...
			assert tuple0 != null;
			scope = new WritableScopeImpl(scope, jetNamespace, RedeclarationHandler.THROW_EXCEPTION, "Builtin classes scope: needed to analyze builtins which depend on Unit type alias");
			scope.changeLockLevel(WritableScope.LockLevel.BOTH);
	...
	private List<NapileFile> getJetFiles(String dir)
	{
		...
		return ContainerUtil.mapNotNull(psiDirectory.getFiles(), new Function<PsiFile, NapileFile>()
		{
			@Override
			public NapileFile fun(PsiFile file)
			{
				return file instanceof NapileFile ? (NapileFile) file : null;
			}
		});
	}
	...
	@Nullable
	public PsiElement resolveStandardLibrarySymbol(@NotNull BindingContext originalContext, @Nullable NapileReferenceExpression referenceExpression)
	{
		ensureInitialized();
	...
	private static class FakeJetNamespaceDescriptor extends NamespaceDescriptorImpl
	{
		...
		{
			super(new NamespaceDescriptorImpl(new ModuleDescriptor(Name.special("<fake_module>")), Collections.<AnnotationDescriptor>emptyList(), Name.special("<root>")), Collections.<AnnotationDescriptor>emptyList(), JetStandardClasses.STANDARD_CLASSES_NAMESPACE.getName());
		}
