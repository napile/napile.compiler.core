 ...
 * @author svtk
 */
public class RemovePartsFromPropertyFix extends JetIntentionAction<NapileProperty>
{
	private final boolean removeInitializer;
	private final boolean removeGetter;
	private final boolean removeSetter;

	private RemovePartsFromPropertyFix(@NotNull NapileProperty element, boolean removeInitializer, boolean removeGetter, boolean removeSetter)
	{
		super(element);
	...
	private RemovePartsFromPropertyFix(@NotNull NapileProperty element)
	{
		this(element, element.getInitializer() != null, element.getGetter() != null && element.getGetter().getBodyExpression() != null, element.getSetter() != null && element.getSetter().getBodyExpression() != null);
	}
	...
	public void invoke(@NotNull Project project, Editor editor, PsiFile file) throws IncorrectOperationException
	{
		if(!(file instanceof NapileFile))
			return;
		JetType type = QuickFixUtil.getDeclarationReturnType(element);
		NapileProperty newElement = (NapileProperty) element.copy();
		NapilePropertyAccessor getter = newElement.getGetter();
		if(removeGetter && getter != null)
		{
			newElement.deleteChildInternal(getter.getNode());
		}
		NapilePropertyAccessor setter = newElement.getSetter();
		if(removeSetter && setter != null)
		{
			newElement.deleteChildInternal(setter.getNode());
		}
		NapileExpression initializer = newElement.getInitializer();
		...
		element = (NapileProperty) element.replace(newElement);
		if(typeToAdd != null)
		{
	...
	public static JetIntentionActionFactory createFactory()
	{
		...
		{
			@Override
			public JetIntentionAction<NapileProperty> createAction(Diagnostic diagnostic)
			{
				PsiElement element = diagnostic.getPsiElement();
				assert element instanceof NapileElement;
