...
public class JetLineMarkerProvider implements LineMarkerProvider
{
	...
	{
		NapileFile file = (NapileFile) element.getContainingFile();
		if(file == null)
			return null;
	...
	{
		NapileFile file = (NapileFile) elt.getContainingFile();
		assert file != null;
...
/*	private static void collectInheritingClasses(NapileClass element, Collection<LineMarkerInfo> result)
	{
		...
		NapileClassOrObject inheritor = ClassInheritorsSearch.search(element, false).findFirst();
		if(inheritor != null)
		{
