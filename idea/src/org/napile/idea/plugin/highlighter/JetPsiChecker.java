...
public class JetPsiChecker implements Annotator
{
	...
	static void highlightName(@NotNull AnnotationHolder holder, @NotNull PsiElement psiElement, @NotNull TextAttributesKey attributesKey)
	{
		if(isNamesHighlightingEnabled())
		{
			if(PsiTreeUtil.getParentOfType(psiElement, NapileAnnotationEntry.class) != null)
				holder.createInfoAnnotation(new TextRange(psiElement.getNode().getStartOffset() - 1, psiElement.getNode().getStartOffset() + psiElement.getTextLength() + 1), null).setTextAttributes(attributesKey);
			else
	...
	private static void registerDiagnosticAnnotations(@NotNull Diagnostic diagnostic, @NotNull Set<PsiElement> redeclarations, @NotNull final AnnotationHolder holder)
	{
	...
	@Nullable
	private static Annotation markRedeclaration(@NotNull Set<PsiElement> redeclarations, @NotNull Diagnostic redeclarationDiagnostic, @NotNull AnnotationHolder holder)
	{
		if(!redeclarations.add(redeclarationDiagnostic.getPsiElement()))
			return null;
