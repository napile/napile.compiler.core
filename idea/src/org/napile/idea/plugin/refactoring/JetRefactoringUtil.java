...
public class JetRefactoringUtil
{
	...
	{
		void run(@Nullable NapileExpression expression);
	}
	...
	private static void smartSelectExpression(@NotNull Editor editor, @NotNull PsiFile file, int offset, @NotNull final SelectExpressionCallback callback) throws IntroduceRefactoringException
	{
		if(offset < 0)
				...
				!(element instanceof NapileNamedFunction) && !(element instanceof NapileClassBody))
		{
			if(element instanceof NapileExpression && !(element instanceof NapileStatementExpression))
			{
				...
				else if(element.getParent() instanceof NapileCallElement)
				{
					addExpression = false;
				}
				else if(element.getParent() instanceof NapileOperationExpression)
				{
					NapileOperationExpression operationExpression = (NapileOperationExpression) element.getParent();
				...
				if(addExpression)
				{
					...
					if(expressionType == null || !(expressionType instanceof NamespaceType) && !JetTypeChecker.INSTANCE.equalTypes(JetStandardClasses.getTupleType(), expressionType))
					{
		...
		for(NapileExpression expression : expressions)
		{
		...
		list.setCellRenderer(new DefaultListCellRenderer()
		{
			...
			public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus)
			{
				...
				NapileExpression element = (NapileExpression) value;
				if(element.isValid())
				{
		...
		list.addListSelectionListener(new ListSelectionListener()
		{
			...
			public void valueChanged(ListSelectionEvent e)
			{
				...
				if(selectedIndex < 0)
					return;
				NapileExpression expression = (NapileExpression) model.get(selectedIndex);
				...
				setRequestFocus(true).setItemChoosenCallback(new Runnable()
		{
			...
			public void run()
			{
				callback.run((NapileExpression) list.getSelectedValue());
			}
	...
	public static String getExpressionShortText(@NotNull NapileExpression expression)
	...
	@Nullable
	public static NapileExpression findExpression(@NotNull Editor editor, @NotNull PsiFile file, int startOffset, int endOffset) throws IntroduceRefactoringException
	{
		PsiElement element = PsiTreeUtil.findElementOfClassAtRange(file, startOffset, endOffset, NapileExpression.class);
		...
		else if(!(element instanceof NapileExpression))
		{
			throw new IntroduceRefactoringException(JetRefactoringBundle.message("cannot.refactor.not.expression"));
		}
		...
		return (NapileExpression) element;
	}
	...
	public static PsiElement[] findStatements(@NotNull PsiFile file, int startOffset, int endOffset)
	{
		...
		while(true)
		{
			if(parent instanceof NapileBlockExpression)
				break;
			if(parent == null || parent instanceof NapileFile)
				return PsiElement.EMPTY_ARRAY;
		...
		{
			if(!(element instanceof NapileExpression || element instanceof PsiWhiteSpace || element instanceof PsiComment))
			{
				return PsiElement.EMPTY_ARRAY;
			}
		}
