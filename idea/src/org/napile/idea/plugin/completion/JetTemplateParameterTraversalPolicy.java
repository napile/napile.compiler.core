...
public class JetTemplateParameterTraversalPolicy implements TemplateParameterTraversalPolicy
{
	...
	{
		return file instanceof NapileFile && PsiTreeUtil.findChildOfType(file, NapileIdeTemplate.class) != null;
	}
	...
	private static PsiElement goToNextPrevElement(@NotNull PsiElement element, boolean next)
	{
		if(next)
		{
			...
			if(nextLeaf == null)
			{
				PsiElement root = PsiTreeUtil.getTopmostParentOfType(element, NapileFile.class);
				assert root != null;
