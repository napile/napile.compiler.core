...
public class JetBlock extends AbstractBlock
{
	...
	private static final TokenSet CODE_BLOCKS = TokenSet.create(JetNodeTypes.BLOCK, JetNodeTypes.CLASS_BODY, JetNodeTypes.FUNCTION_LITERAL_EXPRESSION, JetNodeTypes.FUNCTION_LITERAL);

	// private static final List<IndentWhitespaceRule>

	public JetBlock(@NotNull ASTNode node, Alignment alignment, Indent indent, Wrap wrap, CodeStyleSettings settings, SpacingBuilder spacingBuilder)
	{
	...
	protected Map<ASTNode, Alignment> createChildrenAlignments()
	{
		...
		{
			strategy = getAlignmentForChildInParenthesis(jetCommonSettings.ALIGN_MULTILINE_PARAMETERS, JetNodeTypes.VALUE_PARAMETER, JetTokens.COMMA, jetCommonSettings.ALIGN_MULTILINE_METHOD_BRACKETS, JetTokens.LPAR, JetTokens.RPAR);
		}
		else if(parentType == JetNodeTypes.VALUE_ARGUMENT_LIST)
		{
			strategy = getAlignmentForChildInParenthesis(jetCommonSettings.ALIGN_MULTILINE_PARAMETERS_IN_CALLS, JetNodeTypes.VALUE_ARGUMENT, JetTokens.COMMA, jetCommonSettings.ALIGN_MULTILINE_METHOD_BRACKETS, JetTokens.LPAR, JetTokens.RPAR);
		}
	...
	static ASTIndentStrategy[] INDENT_RULES = new ASTIndentStrategy[]{
			ASTIndentStrategy.forNode("No indent for braces in blocks").in(JetNodeTypes.BLOCK, JetNodeTypes.CLASS_BODY, JetNodeTypes.FUNCTION_LITERAL_EXPRESSION).forType(JetTokens.RBRACE, JetTokens.LBRACE).set(Indent.getNoneIndent()),

			ASTIndentStrategy.forNode("Indent for block content").in(JetNodeTypes.BLOCK, JetNodeTypes.CLASS_BODY, JetNodeTypes.FUNCTION_LITERAL_EXPRESSION).notForType(JetTokens.RBRACE, JetTokens.LBRACE).set(Indent.getNormalIndent()),

			ASTIndentStrategy.forNode("Indent for property accessors").in(JetNodeTypes.PROPERTY).forType(JetNodeTypes.PROPERTY_ACCESSOR).set(Indent.getNormalIndent()),

			ASTIndentStrategy.forNode("For a single statement if 'for'").in(JetNodeTypes.BODY).notForType(JetNodeTypes.BLOCK).set(Indent.getNormalIndent()),

			ASTIndentStrategy.forNode("For the entry in when").forType(JetNodeTypes.WHEN_ENTRY).set(Indent.getNormalIndent()),

			ASTIndentStrategy.forNode("For single statement in THEN and ELSE").in(JetNodeTypes.THEN, JetNodeTypes.ELSE).notForType(JetNodeTypes.BLOCK).set(Indent.getNormalIndent()),

			ASTIndentStrategy.forNode("Indent for parts").in(JetNodeTypes.PROPERTY, JetNodeTypes.METHOD).set(Indent.getContinuationWithoutFirstIndent()),
	};
