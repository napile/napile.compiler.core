...
public class JetIntroduceVariableHandler extends JetIntroduceHandlerBase
{
					...
					public void run()
					{
						...
						if(property != null)
						{
							...
							if(isInplaceAvailableOnDataContext)
							{
								...
								JetInplaceVariableIntroducer variableIntroducer = new JetInplaceVariableIntroducer(property, editor, project, INTRODUCE_VARIABLE, references.toArray(new JetExpression[references.size()]), reference.get(), finalReplaceOccurrence, property, /*todo*/false, /*todo*/false, expressionType, finalNoTypeInference);
	...
	private static boolean isBadContainerNode(JetContainerNode parent, PsiElement place)
	{
		...
		else if(parent.getParent() instanceof JetLoopExpression && ((JetLoopExpression) parent.getParent()).getBody() != place)
		{
			return true;
		}
		return false;
