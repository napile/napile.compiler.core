...
public class ScriptDescriptor extends DeclarationDescriptorNonRootImpl
{
	...
	public ScriptDescriptor(@Nullable DeclarationDescriptor containingDeclaration, int priority, JetScript script, JetScope scriptScope)
	{
		...
		classDescriptor = new ClassDescriptorImpl(containingDeclaration, Collections.<AnnotationDescriptor>emptyList(), Modality.FINAL, new FqName(className).shortName());
		...
		classDescriptor.initialize(false, Collections.<TypeParameterDescriptor>emptyList(), Collections.singletonList(NapileLangPackage.ANY.getTypeSafe(classScope, false)), classScope, new HashSet<ConstructorDescriptor>(), null);
	...
	public void initialize(@NotNull JetType returnType, JetScript declaration, BindingContext bindingContext)
	{
		...
		PropertyDescriptor propertyDescriptor = new PropertyDescriptor(classDescriptor, Collections.<AnnotationDescriptor>emptyList(), Modality.FINAL, Visibilities.PUBLIC, false, false, Name.identifier(ScriptNameUtil.LAST_EXPRESSION_VALUE_FIELD_NAME), CallableMemberDescriptor.Kind.DECLARATION);
		...
		for(JetDeclaration jetDeclaration : declaration.getDeclarations())
		{
			if(jetDeclaration instanceof JetProperty)
			{
				...
				propertyDescriptor = new PropertyDescriptor(classDescriptor, Collections.<AnnotationDescriptor>emptyList(), Modality.FINAL, propertyDescriptor.getVisibility(), false, false, descriptor.getName(), CallableMemberDescriptor.Kind.DECLARATION);
			...
			else if(jetDeclaration instanceof JetNamedFunction)
			{
				...
				assert descriptor != null;
				SimpleFunctionDescriptor copy = descriptor.copy(classDescriptor, descriptor.getModality(), false, CallableMemberDescriptor.Kind.DECLARATION, false);
	...
	public void setValueParameters(@NotNull List<ValueParameterDescriptor> valueParameters)
	{
		...
		ConstructorDescriptorImpl constructorDescriptor = new ConstructorDescriptorImpl(classDescriptor, Collections.<AnnotationDescriptor>emptyList(), true).initialize(Collections.<TypeParameterDescriptor>emptyList(), valueParameters, Visibilities.PUBLIC);
		...
		{
			PropertyDescriptor propertyDescriptor = new PropertyDescriptor(classDescriptor, Collections.<AnnotationDescriptor>emptyList(), Modality.FINAL, Visibilities.PUBLIC, false, false, parameter.getName(), CallableMemberDescriptor.Kind.DECLARATION);
