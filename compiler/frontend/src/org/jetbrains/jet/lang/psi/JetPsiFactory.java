...
public class JetPsiFactory
{
	...
	{
		return (JetFile) PsiFileFactory.getInstance(project).createFileFromText(fileName, JetFileType.INSTANCE, text, LocalTimeCounter.currentTime(), false);
	}
	...
	private static <T> T createDeclaration(Project project, String text, Class<T> clazz)
	{
		...
		List<JetClass> dcls = file.getDeclarations();
