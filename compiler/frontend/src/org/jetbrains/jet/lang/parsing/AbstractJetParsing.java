...
import static org.jetbrains.jet.lexer.JetTokens.WHITE_SPACE_OR_COMMENT_BIT_SET;
...
/*package*/ abstract class AbstractJetParsing
{
	...
	{
		IElementType tt = tt();
		if(recoverySet == null || recoverySet.contains(tt) || (recoverySet.contains(EOL_OR_SEMICOLON) && (eof() || tt == SEMICOLON || myBuilder.newlineBeforeCurrentToken())))
		{
			error(message);
		}
		else
