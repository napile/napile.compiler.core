...
import static org.jetbrains.jet.lexer.JetTokens.QUEST;
import static org.jetbrains.jet.lexer.JetTokens.SAFE_ACCESS;
import static org.jetbrains.jet.lexer.JetTokens.SHEBANG_COMMENT;
...
public class SemanticWhitespaceAwarePsiBuilderImpl extends PsiBuilderAdapter implements SemanticWhitespaceAwarePsiBuilder
{
	...
	public boolean newlineBeforeCurrentToken()
	{
		...
		for(int i = 1; i <= getCurrentOffset(); i++)
		{
			...
			if(previousToken == JetTokens.BLOCK_COMMENT || previousToken == JetTokens.DOC_COMMENT || previousToken == JetTokens.EOL_COMMENT || previousToken == SHEBANG_COMMENT)
			{
				continue;
			}
	...
	private IElementType getJoinedTokenType(IElementType rawTokenType, int rawLookupSteps)
	{
		if(rawTokenType == QUEST)
		{
			...
			if(nextRawToken == DOT)
				return SAFE_ACCESS;
			if(nextRawToken == COLON)
				return ELVIS;
		}
		else if(rawTokenType == EXCL)
		{
			IElementType nextRawToken = rawLookup(rawLookupSteps);
			if(nextRawToken == EXCL)
				return EXCLEXCL;
		}
		return rawTokenType;
	}
