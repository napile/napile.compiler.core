...
public class ConstraintSystemImpl implements ConstraintSystem
{
	...
	private void addConstraint(@NotNull ConstraintKind constraintKind, @NotNull JetType subjectType, @Nullable JetType constrainingType, @NotNull ConstraintPosition constraintPosition)
	{
		...
		for(int i = 0; i < subjectArguments.size(); i++)
		{
			...
			addConstraint(ConstraintKind.fromVariance(parameters.get(i).getVariance()), subjectArguments.get(i).getType(), constrainingArguments.get(i).getType(), constraintPosition);
		}
	}
