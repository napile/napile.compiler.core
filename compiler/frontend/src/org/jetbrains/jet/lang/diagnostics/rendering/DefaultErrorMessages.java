...
import static org.jetbrains.jet.lang.diagnostics.rendering.Renderers.AMBIGUOUS_CALLS;
import static org.jetbrains.jet.lang.diagnostics.rendering.Renderers.ELEMENT_TEXT;
...
import static org.jetbrains.jet.lang.diagnostics.rendering.Renderers.TYPE_INFERENCE_CONFLICTING_SUBSTITUTIONS_RENDERER;
import static org.jetbrains.jet.lang.diagnostics.rendering.Renderers.TYPE_INFERENCE_NO_INFORMATION_FOR_PARAMETER_RENDERER;
import static org.jetbrains.jet.lang.diagnostics.rendering.Renderers.TYPE_INFERENCE_TYPE_CONSTRUCTOR_MISMATCH_RENDERER;
import static org.jetbrains.jet.lang.diagnostics.rendering.Renderers.TYPE_INFERENCE_UPPER_BOUND_VIOLATED_RENDERER;
import static org.jetbrains.jet.lang.diagnostics.rendering.Renderers.commaSeparated;
...
public class DefaultErrorMessages
{
	...
	static
	{
		...
		MAP.put(CANNOT_INFER_PARAMETER_TYPE, "Cannot infer a type for this parameter. To specify it explicitly use the {(p : Type) => ...} notation");

		MAP.put(NO_BACKING_FIELD_ABSTRACT_PROPERTY, "This property doesn't have a backing field, because it's abstract");
		MAP.put(NO_BACKING_FIELD_CUSTOM_ACCESSORS, "This property doesn't have a backing field, because it has custom accessors without reference to the backing field");
		...
		MAP.put(INITIALIZATION_USING_BACKING_FIELD_CUSTOM_SETTER, "This property has a custom setter, so initialization using backing field required", NAME);
		MAP.put(INITIALIZATION_USING_BACKING_FIELD_OPEN_SETTER, "Setter of this property can be overridden, so initialization using backing field required", NAME);
		...
		MAP.put(HAS_NEXT_PROPERTY_TYPE_MISMATCH, "The type of ''iterator().hasNext'' property of the loop range must be jet.Boolean, but is {0}", RENDER_TYPE);
		MAP.put(HAS_NEXT_FUNCTION_TYPE_MISMATCH, "The ''iterator().hasNext()'' function of the loop range must return jet.Boolean, but returns {0}", RENDER_TYPE);
		...
		MAP.put(ASSIGNMENT_TYPE_MISMATCH, "Expected a value of type {0}. Assignment operation is not an expression, so it does not return any value", RENDER_TYPE);
		MAP.put(IMPLICIT_CAST_TO_UNIT_OR_ANY, "Type was casted to ''{0}''. Please specify ''{0}'' as expected type, if you mean such cast", RENDER_TYPE);
		...
		MAP.put(RETURN_TYPE_MISMATCH_ON_OVERRIDE, "Return type of ''{0}'' is not a subtype of the return type of overridden member {1}", NAME, DescriptorRenderer.TEXT);

		MAP.put(VAR_OVERRIDDEN_BY_VAL, "Var-property {0} cannot be overridden by val-property {1}", DescriptorRenderer.TEXT, DescriptorRenderer.TEXT);

		MAP.put(ABSTRACT_MEMBER_NOT_IMPLEMENTED, "{0} must be declared abstract or implement abstract member {1}", RENDER_CLASS_OR_OBJECT, DescriptorRenderer.TEXT);

		MAP.put(MANY_IMPL_MEMBER_NOT_IMPLEMENTED, "{0} must override {1} because it inherits many implementations of it", RENDER_CLASS_OR_OBJECT, DescriptorRenderer.TEXT);
		...
		MAP.put(UNSAFE_INFIX_CALL, "Infix call corresponds to a dot-qualified call ''{0}.{1}({2})'' which is not allowed on a nullable receiver ''{0}''. " + "Use '?.'-qualified call instead", TO_STRING, TO_STRING, TO_STRING);
