...
public abstract class AbstractDiagnostic<E extends PsiElement> implements ParametrizedDiagnostic<E>
{
	...
	{
		if(!getFactory().isValid(this))
			return false;
		if(hasSyntaxErrors(psiElement))
			return false;
		if(psiElement.getNode().findChildByType(JetNodeTypes.IDE_TEMPLATE_EXPRESSION) != null)
			return false;
		return true;
	}
	...
	private static boolean hasSyntaxErrors(@NotNull PsiElement psiElement)
	{
		...
		if(lastChild != null && hasSyntaxErrors(lastChild))
			return true;

		PsiElement[] children = psiElement.getChildren();
		if(children.length > 0 && hasSyntaxErrors(children[children.length - 1]))
			return true;
