...
public class DefaultErrorMessages
{
	...
	static
	{
		...
		MAP.put(INCOMPATIBLE_MODIFIERS, "Incompatible modifiers: ''{0}''", new Renderer<Collection<NapileKeywordToken>>()
		{
			@NotNull
			@Override
			public String render(@NotNull Collection<NapileKeywordToken> tokens)
			{
				StringBuilder sb = new StringBuilder();
				for(NapileKeywordToken token : tokens)
				{
					if(sb.length() != 0)
					{
						sb.append(" ");
		...
		MAP.put(HAS_NEXT_PROPERTY_TYPE_MISMATCH, "The type of ''iterator().hasNext'' property of the loop range must be idea.Boolean, but is {0}", RENDER_TYPE);
		MAP.put(HAS_NEXT_FUNCTION_TYPE_MISMATCH, "The ''iterator().hasNext()'' function of the loop range must return idea.Boolean, but returns {0}", RENDER_TYPE);
		...
		MAP.put(EXPRESSION_EXPECTED, "{0} is not an expression, and only expression are allowed here", new Renderer<NapileExpression>()
		{
			@NotNull
			@Override
			public String render(@NotNull NapileExpression expression)
			{
		...
		MAP.put(EQUALITY_NOT_APPLICABLE, "Operator ''{0}'' cannot be applied to ''{1}'' and ''{2}''", new Renderer<NapileSimpleNameExpression>()
		{
			@NotNull
			@Override
			public String render(@NotNull NapileSimpleNameExpression nameExpression)
			{
				//noinspection ConstantConditions
