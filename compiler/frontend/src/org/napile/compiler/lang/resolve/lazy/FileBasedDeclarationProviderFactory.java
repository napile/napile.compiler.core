...
public class FileBasedDeclarationProviderFactory implements DeclarationProviderFactory
{
	...
	private final Multimap<FqName, NapileFile> filesByPackage = HashMultimap.create();
	...
	//private final Map<NapileClassOrObject, ClassMemberDeclarationProvider> classMemberDeclarationProviders = Maps.newHashMap();

	private final Predicate<FqName> isPackageDeclaredExternally;

	private boolean indexed = false;

	public FileBasedDeclarationProviderFactory(@NotNull Collection<NapileFile> files)
	{
		this(files, Predicates.<FqName>alwaysFalse());
	}

	public FileBasedDeclarationProviderFactory(@NotNull Collection<NapileFile> files, Predicate<FqName> isPackageDeclaredExternally)
	{
		this.allFiles = files;
	...
	@NotNull
	@Override
	public ClassMemberDeclarationProvider getClassMemberDeclarationProvider(@NotNull NapileClassLikeInfo classLikeInfo)
	{
		createIndex();

		//NapileClassOrObject classOrObject = classLikeInfo.getCorrespondingClassOrObject();
