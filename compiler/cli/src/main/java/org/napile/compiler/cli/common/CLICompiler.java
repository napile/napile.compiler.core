...
import org.napile.compiler.cli.jvm.compiler.CompileEnvironmentException;
import org.napile.compiler.cli.jvm.compiler.CompileEnvironmentUtil;
...
public abstract class CLICompiler<A extends CompilerArguments>
{
	...
	protected void printVersionIfNeeded(@NotNull PrintStream errStream, @NotNull A arguments, @NotNull MessageRenderer messageRenderer)
	{
		...
		{
			String versionMessage = messageRenderer.render(CompilerMessageSeverity.INFO, "Kotlin Compiler version " + CompilerVersion.VERSION, CompilerMessageLocation.NO_LOCATION);
	...
	{
		ExitCode exitCode = doMainNoExit(System.out, compiler, args);
		if(exitCode != OK)
		{
	...
	@NotNull
	public static ExitCode doMainNoExit(@NotNull PrintStream printStream, @NotNull CLICompiler compiler, @NotNull String[] args)
	{
		try
		{
			ExitCode rc = compiler.exec(printStream, args);
			if(rc != OK)
			{
