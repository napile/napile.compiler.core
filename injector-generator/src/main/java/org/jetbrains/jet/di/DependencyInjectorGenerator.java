...
public class DependencyInjectorGenerator
{
	...
	private void generateImportDirective(PrintStream out, Class<?> type, String injectorPackageName)
	{
		...
		if("java.lang".equals(importedPackageName) || injectorPackageName.equals(importedPackageName) || !imports.add(type))
		{
			return;
		}
	...
	// Constructor parameters
		...
		{
			out.print("\tpublic " + injectorClassName + "(");
		...
		if(lazy)
		{
			...
			for(Parameter parameter : parameters)
			{
				out.println("\t\t" + "this." + parameter.getField().getName() + " = " + parameter.getName() + ";");
			}
		}
		else
		{
			...
			for(Field field : fields)
			{
				...
				String prefix = "this.";
				out.println("\t\t" + prefix + field.getName() + " = " + field.getInitialization().renderAsCode() + ";");
				//}
			}
			out.println();
			...
			for(Field field : fields)
			{
				...
				{
					out.println("\t\t" + field.getName() + "." + postConstruct.getName() + "();");
				}
			...
			if(lazy && !backsParameter.contains(field))
			{
				...
				for(SetterDependency dependency : field.getDependencies())
				{
					out.println(indent2 +
							...
							dependency.getSetterName() +
							"(" +
							dependency.getDependency().getGetterName() +
							"());");
				}
	...
	private void satisfyDependenciesFor(Field field, Field neededFor)
	{
		...
		{
			if(method.getAnnotation(javax.inject.Inject.class) == null || !method.getName().startsWith("set") || method.getParameterTypes().length != 1)
			{
				continue;
			}
