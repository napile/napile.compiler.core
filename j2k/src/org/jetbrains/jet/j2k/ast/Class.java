...
import static org.jetbrains.jet.j2k.util.AstUtil.joinNodes;
	...
	List<Function> secondaryConstructorsAsStaticInitFunction()
	{
		...
		for(Member m : myMembers)
		{
			if(m.getKind() == Kind.CONSTRUCTOR && !((Constructor) m).isPrimary())
			{
				...
				result.add(new Function(new IdentifierImpl("init"), modifiers, new ClassType(myName, typeParameters, false), typeParameters, f.getParams(), block));
			}
		}
		return result;
	}
