...
import static org.jetbrains.jet.j2k.visitors.TypeVisitor.JAVA_LANG_STRING;
...
public class ExpressionVisitor extends StatementVisitor
{
	...
	public void visitArrayAccessExpression(@NotNull PsiArrayAccessExpression expression)
	{
		...
		myResult = new ArrayAccessExpression(getConverter().expressionToExpression(expression.getArrayExpression()), getConverter().expressionToExpression(expression.getIndexExpression()));
	}
	...
	public void visitArrayInitializerExpression(@NotNull PsiArrayInitializerExpression expression)
	{
		...
		myResult = new ArrayInitializerExpression(getConverter().typeToType(expression.getType()), getConverter().expressionsToExpressionList(expression.getInitializers()));
	}
	...
	public void visitAssignmentExpression(@NotNull PsiAssignmentExpression expression)
	{
		...
		{
			myResult = new AssignmentExpression(getConverter().expressionToExpression(expression.getLExpression()), new BinaryExpression(getConverter().expressionToExpression(expression.getLExpression()), getConverter().expressionToExpression(expression.getRExpression()), secondOp), "=");
		}
		else
		{
			myResult = new AssignmentExpression(getConverter().expressionToExpression(expression.getLExpression()), getConverter().expressionToExpression(expression.getRExpression()), expression.getOperationSign().getText() // TODO
			);
		}
	}
	...
	public void visitBinaryExpression(@NotNull PsiBinaryExpression expression)
	{
		...
		{
			myResult = new DummyMethodCallExpression(getConverter().expressionToExpression(expression.getLOperand()), "ushr", getConverter().expressionToExpression(expression.getROperand()));
		}
		else
		{
			myResult = new BinaryExpression(getConverter().expressionToExpression(expression.getLOperand()), getConverter().expressionToExpression(expression.getROperand()), getOperatorString(expression.getOperationSign().getTokenType()), getConverter().createConversions(expression, PsiType.BOOLEAN));
		}
	}
	...
	public void visitConditionalExpression(@NotNull PsiConditionalExpression expression)
	{
		...
		Expression e = type != null ? getConverter().createSureCallOnlyForChain(condition, type) : getConverter().expressionToExpression(condition);
		myResult = new ParenthesizedExpression(new IfStatement(e, getConverter().expressionToExpression(expression.getThenExpression()), getConverter().expressionToExpression(expression.getElseExpression())));
	}
	...
	public void visitInstanceOfExpression(@NotNull PsiInstanceOfExpression expression)
	{
		...
		myResult = new IsOperator(getConverter().expressionToExpression(expression.getOperand()), getConverter().elementToElement(expression.getCheckType()));
	}
	...
	{
		return new ArrayWithoutInitializationExpression(getConverter().typeToType(expression.getType(), true), getConverter().expressionsToExpressionList(expression.getArrayDimensions()));
	}
	...
	public void visitParenthesizedExpression(@NotNull PsiParenthesizedExpression expression)
	{
		...
		myResult = new ParenthesizedExpression(getConverter().expressionToExpression(expression.getExpression()));
	}
	...
	public void visitPostfixExpression(@NotNull PsiPostfixExpression expression)
	{
		...
		myResult = new PostfixOperator(getOperatorString(expression.getOperationSign().getTokenType()), getConverter().expressionToExpression(expression.getOperand()));
	}
	...
	public void visitPrefixExpression(@NotNull PsiPrefixExpression expression)
	{
		...
		{
			myResult = new DummyMethodCallExpression(new ParenthesizedExpression(getConverter().expressionToExpression(expression.getOperand())), "inv", Expression.EMPTY_EXPRESSION);
		}
		else
		{
			myResult = new PrefixOperator(getOperatorString(expression.getOperationSign().getTokenType()), getConverter().expressionToExpression(expression.getOperand()));
		}
	}
	...
	public void visitSuperExpression(@NotNull PsiSuperExpression expression)
	{
		...
		myResult = new SuperExpression(qualifier != null ? new IdentifierImpl(qualifier.getQualifiedName()) : Identifier.EMPTY_IDENTIFIER);
	}
	...
	public void visitThisExpression(@NotNull PsiThisExpression expression)
	{
		...
		final PsiJavaCodeReferenceElement qualifier = expression.getQualifier();
		myResult = new ThisExpression(qualifier != null ? new IdentifierImpl(qualifier.getQualifiedName()) : Identifier.EMPTY_IDENTIFIER);
	}
	...
	public void visitTypeCastExpression(@NotNull PsiTypeCastExpression expression)
	{
		...
		if(castType != null)
		{
			myResult = new TypeCastExpression(getConverter().typeToType(castType.getType()), getConverter().expressionToExpression(expression.getOperand()));
		}
	}
	...
	public void visitPolyadicExpression(@NotNull PsiPolyadicExpression expression)
	{
		...
		myResult = new PolyadicExpression(getConverter().expressionsToExpressionList(expression.getOperands()), getOperatorString(expression.getOperationTokenType()), getConverter().createConversions(expression, PsiType.BOOLEAN));
	}
}
