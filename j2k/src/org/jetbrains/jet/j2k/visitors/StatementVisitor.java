...
public class StatementVisitor extends ElementVisitor
{
	...
	public void visitAssertStatement(@NotNull PsiAssertStatement statement)
	{
		...
		myResult = new AssertStatement(getConverter().expressionToExpression(statement.getAssertCondition()), getConverter().expressionToExpression(statement.getAssertDescription()));
	}
	...
	public void visitBlockStatement(@NotNull PsiBlockStatement statement)
	{
		...
		myResult = new Block(getConverter().statementsToStatementList(statement.getCodeBlock().getStatements()), true);
	}
	...
	public void visitBreakStatement(@NotNull PsiBreakStatement statement)
	{
		...
		{
			myResult = new BreakStatement(identifierToIdentifier(statement.getLabelIdentifier()));
		}
	}
	...
	public void visitContinueStatement(@NotNull PsiContinueStatement statement)
	{
		...
		{
			myResult = new ContinueStatement(identifierToIdentifier(statement.getLabelIdentifier()));
		}
	}
	...
	public void visitDeclarationStatement(@NotNull PsiDeclarationStatement statement)
	{
		...
		myResult = new DeclarationStatement(getConverter().elementsToElementList(statement.getDeclaredElements()));
	}
	...
	public void visitDoWhileStatement(@NotNull PsiDoWhileStatement statement)
	{
		...
		@SuppressWarnings("ConstantConditions") Expression expression = condition != null && condition.getType() != null ? getConverter().createSureCallOnlyForChain(condition, condition.getType()) : getConverter().expressionToExpression(condition);
		myResult = new DoWhileStatement(expression, getConverter().statementToStatement(statement.getBody()));
	}
	...
	public void visitExpressionListStatement(@NotNull PsiExpressionListStatement statement)
	{
		...
		myResult = new ExpressionListStatement(getConverter().expressionsToExpressionList(statement.getExpressionList().getExpressions()));
	}
	...
	public void visitForStatement(@NotNull PsiForStatement statement)
	{
				...
				initialization instanceof PsiDeclarationStatement && initialization.getFirstChild() == initialization.getLastChild() && condition != null && update != null && update.getChildren().length == 1 && isPlusPlusExpression(update.getChildren()[0]) && operationTokenType != null && (operationTokenType == JavaTokenType.LT || operationTokenType == JavaTokenType.LE) && initialization.getFirstChild() != null && initialization.getFirstChild() instanceof PsiLocalVariable && firstChild != null && firstChild.getNameIdentifier() != null && onceWritableIterator)
		{
			...
			myResult = new ForeachWithRangeStatement(new IdentifierImpl(firstChild.getName()), getConverter().expressionToExpression(firstChild.getInitializer()), endExpression, getConverter().statementToStatement(body));
		}
		else
		{ // common case: while loop instead of for loop
			List<Statement> forStatements = new LinkedList<Statement>();
			forStatements.add(getConverter().statementToStatement(initialization));
			forStatements.add(new WhileStatement(getConverter().expressionToExpression(condition), new Block(Arrays.asList(getConverter().statementToStatement(body), new Block(Arrays.asList(getConverter().statementToStatement(update)))))));
	...
	public void visitForeachStatement(@NotNull PsiForeachStatement statement)
	{
		...
		myResult = new ForeachStatement(getConverter().parameterToParameter(statement.getIterationParameter()), getConverter().expressionToExpression(statement.getIteratedValue()), getConverter().statementToStatement(statement.getBody()));
	}
	...
	public void visitIfStatement(@NotNull PsiIfStatement statement)
	{
		...
		PsiExpression condition = statement.getCondition();
		@SuppressWarnings("ConstantConditions") Expression expression = condition != null && condition.getType() != null ? getConverter().createSureCallOnlyForChain(condition, condition.getType()) : getConverter().expressionToExpression(condition);
		myResult = new IfStatement(expression, getConverter().statementToStatement(statement.getThenBranch()), getConverter().statementToStatement(statement.getElseBranch()));
	}
	...
	public void visitLabeledStatement(@NotNull PsiLabeledStatement statement)
	{
		...
		myResult = new LabelStatement(identifierToIdentifier(statement.getLabelIdentifier()), getConverter().statementToStatement(statement.getStatement()));
	}
	...
	public void visitSwitchLabelStatement(@NotNull PsiSwitchLabelStatement statement)
	{
		...
		myResult = statement.isDefaultCase() ? new DefaultSwitchLabelStatement() : new SwitchLabelStatement(getConverter().expressionToExpression(statement.getCaseValue()));
	}
	...
	public void visitSwitchStatement(@NotNull PsiSwitchStatement statement)
	{
		...
		myResult = new SwitchContainer(getConverter().expressionToExpression(statement.getExpression()), switchBodyToCases(statement.getBody()));
	}
	...
	public void visitSynchronizedStatement(@NotNull PsiSynchronizedStatement statement)
	{
		...
		myResult = new SynchronizedStatement(getConverter().expressionToExpression(statement.getLockExpression()), getConverter().blockToBlock(statement.getBody()));
	}
	...
	public void visitThrowStatement(@NotNull PsiThrowStatement statement)
	{
		...
		myResult = new ThrowStatement(getConverter().expressionToExpression(statement.getException()));
	}
	...
	public void visitTryStatement(@NotNull PsiTryStatement statement)
	{
		...
		{
			catches.add(new CatchStatement(getConverter().parameterToParameter(statement.getCatchBlockParameters()[i]), getConverter().blockToBlock(statement.getCatchBlocks()[i], true)));
		}

		myResult = new TryStatement(getConverter().blockToBlock(statement.getTryBlock(), true), catches, getConverter().blockToBlock(statement.getFinallyBlock(), true));
	}
	...
	public void visitWhileStatement(@NotNull PsiWhileStatement statement)
	{
		...
		PsiExpression condition = statement.getCondition();
		@SuppressWarnings("ConstantConditions") Expression expression = condition != null && condition.getType() != null ? getConverter().createSureCallOnlyForChain(condition, condition.getType()) : getConverter().expressionToExpression(condition);
		myResult = new WhileStatement(expression, getConverter().statementToStatement(statement.getBody()));
	}
	...
	public void visitReturnStatement(@NotNull PsiReturnStatement statement)
	{
		...
		Expression expression = returnValue != null && methodReturnType != null ? getConverter().createSureCallOnlyForChain(returnValue, methodReturnType) : getConverter().expressionToExpression(returnValue);
		myResult = new ReturnStatement(expression);
	}
}
